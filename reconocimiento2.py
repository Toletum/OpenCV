...
im = ax.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB), animated=True)
...
def updatefig(*args):
    ...
    for i in range(len(faces)):
        ...
        if prediction[1]<100 :
          ...
          cv2.putText(frame,'%s - %.0f' % (cara,prediction[1]),(x-10, y-10), cv2.FONT_HERSHEY_COMPLEX,1,(0, 255, 0))
        ...
        #Si la prediccion es mayor a 100 no es un reconomiento con la exactitud suficiente
            ...
            cv2.putText(frame, 'Desconocido',(x-10, y-10), cv2.FONT_HERSHEY_PLAIN,2,(0, 255, 0))  
        else:
            cv2.putText(frame, 'Ni idea',(x-10, y-10), cv2.FONT_HERSHEY_PLAIN,2,(0, 255, 0))  
